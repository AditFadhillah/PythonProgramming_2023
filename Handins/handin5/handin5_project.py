...
class ColorMapper:
    ...
    def __init__(self, values, cmap_str='RdBu_r'):
        ...
        :param cmap_str: String argument for the color map, which will be sent to matplotlib.
        """

        # calculate the minimum and maximum absolute values
        max_abs_value = np.max(np.abs(values))

        # set min_value to negative of max_abs_value 
        # and max_value to positive of max_abs_value
        self.min_value = -max_abs_value
        self.max_value = max_abs_value
...
def construct_rectangles(years, anomalies, bottom=0.0, height=1.0):
    """
    Construct a list of rectangles for visualization.

    :param years: Sequence of year values
    :param anomalies: Sequence of anomaly values
    :param bottom: Lower coordinate value of each stripe (default: 0.0)
    :param height: Height of each stripe (default: 1.0)
    :return: List of (year, bottom, width, height, temperature anomaly) tuples
    """
    rectangles = [(year, bottom, 1, height, anomaly) for year, anomaly in zip(years, anomalies)]
    return rectangles
